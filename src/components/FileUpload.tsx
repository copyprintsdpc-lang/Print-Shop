...
import { Upload, X, File, Image, FileText, AlertCircle, CheckCircle, Loader2, Clock } from 'lucide-react'
...
export default function FileUpload({ 
  onFilesChange, 
  maxFiles = 5, 
  maxSize = 100, // Increased to 100MB for artwork files
  ...
  const scheduleChange = useCallback((callback: () => void) => {
  ...
  const updateFiles = useCallback((updater: (files: FileWithStatus[]) => FileWithStatus[]) => {
    setFiles(prev => {
      const next = updater(prev)
      scheduleChange(() => onFilesChange(next))
      return next
    })
  }, [onFilesChange, scheduleChange])

  const uploadFiles = useCallback(async (filesToUpload?: FileWithStatus[]) => {
    if (!autoUpload) {
      return
    }

    if (!uploadEndpoint || uploadEndpoint.trim() === '') {
      console.log('No upload endpoint configured. Files will be uploaded with form submission.')
      return
    }
    ...
    for (let i = 0; i < filesToProcess.length; i++) {
      ...
      updateFiles(prev => prev.map(f => 
        f.id === fileId ? { ...f, uploading: true, progress: 0, error: null } : f
      ))

      try {
        const formData = new FormData()
        formData.append('file', fileObj.file)
        formData.append('type', folder || 'uploads')

        const progressInterval = setInterval(() => {
          setFiles(prev => prev.map(f => 
            f.id === fileId ? { ...f, progress: Math.min(f.progress + 10, 95) } : f
          ))
        }, 200)
        ...
        if (data.ok) {
          const fileUrl = data.previewUrl || data.file?.previewUrl || data.url || data.file?.url || ''
          const permanentUrl = data.url || data.file?.url || ''
          const fileKey = data.key || data.file?.key
          ...
          const errorMessage = data.message || data.code === 'UPLOAD_ERROR' 
            ? 'File upload failed. Files will be uploaded when you submit the form.' 
            : 'Upload failed'
          throw new Error(errorMessage)
        }
      } catch (error) {
        console.error('Upload error:', error)
        const errorMessage = autoUpload 
    ...
    setUploading(false)
  }, [autoUpload, uploadEndpoint, files, folder, updateFiles])
  ...
  }, [files, onFilesChange, autoUpload, uploadFiles])
  ...
  const getStatusDisplay = (fileObj: FileWithStatus) => {
    if (fileObj.uploading) {
      return {
        icon: <Loader2 className="w-4 h-4 animate-spin" />,
        text: `Uploading… ${fileObj.progress}%`,
        textClass: 'text-blue-600',
        barClass: 'bg-blue-600',
        progress: Math.min(fileObj.progress, 100),
        showProgress: true,
      }
    }
    ...
    if (fileObj.error) {
      return {
        ...
        text: fileObj.error || 'Upload failed',
        textClass: 'text-red-600',
        barClass: 'bg-red-500',
        progress: fileObj.progress,
        showProgress: fileObj.progress > 0,
      }
    }
  ...
  return (
    <div className="w-full">
      ...
      {/* File List */}
      {showFileSummary && files.length > 0 && (
              ...
              <div key={index} className={`flex items-center justify-between p-4 rounded-lg border ${
                ...
                <div className="flex items-center space-x-3 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(() => {
                      const status = getStatusDisplay(fileObj)
                      return (
                        <div className="mt-2">
                          <div className={`flex items-center gap-2 text-sm ${status.textClass}`}>
                            {status.icon}
                          ...
                          {status.showProgress && (
                            <div className="w-full bg-gray-200 rounded-full h-1.5 mt-2">
                              <div
                                className={`${status.barClass} h-1.5 rounded-full transition-all duration-300`}
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {!autoUpload && !fileObj.uploading && !fileObj.uploaded && (
                    <button
      ...
      {/* Upload Button */}
      {showUploadButton && !autoUpload && files.length > 0 && files.some(f => !f.uploaded && !f.uploading) && (
        <div className="mt-6">
          <button
            onClick={() => uploadFiles()}
            disabled={uploading}
